...
test("chaque page contient exactement une instance de Claire", async () => {
  for (const page of pages) {
    ...
    assert.equal(matches(html, /href="(assets\/css\/claire-companion\.css\?v=20260912-claire-voice-out-v1)"/g).length, 1, page);
    assert.equal(matches(html, /src="(assets\/js\/claire-companion\.js\?v=20260912-claire-voice-out-v1)"/g).length, 1, page);
...
test("les modules Claire sont versionnés pour éviter un cache 24 h cassé", async () => {
  ...
  for (const file of files) {
    ...
    if (source.includes("claire-core.mjs")) {
      assert.match(source, /claire-core\.mjs\?v=20260912-claire-voice-out-v1/);
    }
  }
});
