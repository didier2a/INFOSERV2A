...
test("la configuration Claire V1 cible uniquement le Worker de préproduction", async () => {
  ...
  assert.match(source, /"PREVIEW_BUILD_ID"\s*:\s*"20260911-claire-send-truth-v1"/);
...
test("la préproduction se distingue par un en-tête de build", async () => {
  const response = await worker.fetch(
    ...
    env({ PREVIEW_BUILD_ID: "20260911-claire-send-truth-v1" })
  );
  assert.equal(response.status, 200);
  assert.equal(response.headers.get("X-InfoServ2A-Preview"), "20260911-claire-send-truth-v1");
