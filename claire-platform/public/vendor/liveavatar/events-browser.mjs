// Minimal browser implementation for the Node "events" import used by the
// official LiveAvatar ESM bundle.
export class EventEmitter {
  ...
  removeListener(name, listener) {
    ...
    const next = listeners.filter((item) => item !== listener && item.listener !== listener);
    if (next.length) this._events.set(name, next);
  ...
  emit(name, ...args) {
    const listeners = this._events.get(name);
    if (!listeners?.length) {
      if (name === "error" && args[0] instanceof Error) throw args[0];
      return false;
    }
...
export function once(emitter, name) {
  return new Promise((resolve, reject) => {
    ...
    const failed = (error) => {
      emitter.removeListener?.(name, done);
      reject(error);
    };
    emitter.once(name, done);
    if (name !== "error") emitter.once("error", failed);
  });
}
