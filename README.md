...
- `send-email.js` envoie réellement contact et devis vers `contact@` / `devis@infoserv2a.pro` **seulement** si le secret `RESEND_API_KEY` est présent. FormSubmit est bloqué depuis un Worker (challenge Cloudflare) : ne plus s’en servir. Claire ne dit « c’est parti » que si cette route confirme l’envoi ;
